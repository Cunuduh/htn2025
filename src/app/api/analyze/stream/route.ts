				...
				try {
					...
					if (spec.allowSearch && hasExaKey()) {
						try {
							...
							if (jsonMatch) {
								try { const parsed = JSON.parse(jsonMatch[0]); if (Array.isArray(parsed.queries)) queries = parsed.queries.slice(0,3).map((q: any)=>String(q).trim()).filter(Boolean); } catch {}
							}
							if (!queries.length) {
							...
							if (queries.length) {
								...
								if (bundles.length) {
									searchContext = bundles.map(b => `\n<search query="${b.query}">\n${b.sources.map(s => `<source url="${s.url}"><![CDATA[${s.title || ''}\n${s.snippet || ''}]]></source>`).join('\n')}\n</search>`).join('');
									for (const b of bundles) {
										send({ type: 'searchResult', agent: spec.id, query: b.query, sources: b.sources.slice(0,5) });
									}
					...
					console.log('[analyze/stream] agent begin', { agent: spec.id, model: 'cerebras-qwen-235b' });
					...
					for await (const rawPart of result.textStream as any) {
						...
						else if (part?.type === 'text-delta') { full += part.text; }
					...
					console.log('[analyze/stream] agent done', { agent: spec.id, chars: parsedMarkdown.length });
					agentOutputs.push({ id: spec.id, name: spec.name, markdown: parsedMarkdown });
